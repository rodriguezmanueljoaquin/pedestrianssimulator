...
def get_figures(e, layer_prefix, figures_are_rectangles):
    figures = []
    if e.dxftype() == 'LINE':
        ...
        if not figures_are_rectangles:
            ...
            figures.append([e[0].dxf.location[0], e[0].dxf.location[1], e[0].dxf.location[2],
...
def get_layer_figures(msp, layer_prefix, expected_types, with_name=False, figures_are_rectangles=False):
    ...
    for e in msp.query().filter(lambda e: regex.match(r"{}*".format(layer_prefix), e.dxf.layer)):
        ...
        for new_figure in get_figures(e, layer_prefix, figures_are_rectangles):
            if with_name:
...
def parse_dxf(in_file_path, out_path):
    ...
    get_layer_figures_and_write_to_file(msp, TARGETS_LAYER_PREFIX, ['CIRCLE'], out_path, 
                with_name=True)

    print("\tParsing servers...")
    # get_layer_figures_and_write_to_file(msp, SERVERS_LAYER_PREFIX, ['LINE', 'POLYLINE'], out_path, 
    #             with_name=True, figures_are_rectangles=True) TODO: add queue support

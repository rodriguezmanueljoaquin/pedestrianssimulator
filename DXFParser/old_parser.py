...
def parse_entities(entities, layer_prefix, expected_types, with_name=False, figures_are_rectangles=False):
    figures = []
    for entity in entities:
        ...
        for new_figure in new_figures:
            if with_name:
                new_figure.insert(0, entity.dxf.layer.split('_', 1)[1])
...
def get_layer_figures(msp, layer_prefix, expected_types, with_name=False, figures_are_rectangles=False):
    figures = parse_entities(msp.query().filter(lambda e: regex.match(r"{}*".format(layer_prefix), e.dxf.layer)), 
                                                layer_prefix, expected_types, with_name, figures_are_rectangles)
